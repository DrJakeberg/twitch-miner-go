...
// itself stays English and machine-greppable; only the notification body is
// phrased for the operator reading it on a phone.
var betHeadlines = map[model.Event]string{
	model.EventBetStart:   "🎰 Wette wird platziert",
	model.EventBetWin:     "🏆 Wette gewonnen",
	model.EventBetLose:    "💸 Wette verloren",
	model.EventBetRefund:  "↩️ Wette erstattet",
	model.EventBetFilters: "🎰 Wette übersprungen",
	model.EventBetFailed:  "🎰 Wette fehlgeschlagen",
}
...
func buildBetBody(event model.Event, msg string, fields []logger.Field) string {
	...
	if channel := formatChannel(fields); channel != "" {
		lines = append(lines, "Kanal: "+channel)
	}
...
func formatPick(fields []logger.Field) string {
	...
	switch {
	case pick != "" && amount != "":
		return fmt.Sprintf("%s · %s Punkte", pick, amount)
	case pick != "":
		return pick
	case amount != "":
		return amount + " Punkte"
	default:
		return ""
	}
}
